...
public class AnimationCreator : MonoBehaviour
{
    ...
    private void CreateAnime(string para)
    {
        ...
        for (x = 0; x < ac.animationClips.Length; x++)
        {
            if (ac.animationClips[x].name.Equals(para))
            {
                animationClip = ac.animationClips[x];
        ...
        AnimationClip animationClip = GetComponent<Animation>().GetClip(para);
        animationClip.ClearCurves();
    ...
    public AnimationClip Create(string para)
    {
        loadMotion();
        AnimationClip animationClip = new AnimationClip();
        animationClip.legacy = true;
        animationClip.name = para;
        ...
        if (para.Equals("attack"))
        {
            ...
            for(int i=0;i<4;i++)
            {
                Debug.Log("RootBone:" + attackPara[i]);
            }
        }
        if (para.Equals("Guard"))
        {
